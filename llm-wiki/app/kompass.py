...
from . import access, bewertung, proposals, stats, wiki

# ---------------------------------------------------------------------------
# Rollen und Schwellen (README "Datenmapping")
# ---------------------------------------------------------------------------

# Anzeigekuerzel -> Rollenschluessel (Kapitel 17)
...
def bewertung_daten(slug: str) -> dict | None:
    """Bewertungsergebnis eines Antrags in der Form, die diese Ansichten erwarten.

    Quelle ist der letzte Orchestrator-Lauf aus Phase 4 (`bewertung.letzter_lauf`),
    nicht mehr die abgeloeste Sammelbewertung ueber `evaluation.py`. Gibt es keinen
    fertigen Lauf, liefert die Funktion None; dann zeigt die Oberflaeche
    "kein Score" statt eines geschaetzten Werts.

    Rueckgabe: {rolle: <Kapitel-17-Objekt>, ...} plus `_gesamtscore` und
    `_anzahl_bewertet` aus der Kapitel-16-Aggregation.
    """
    lauf = bewertung.letzter_lauf(slug)
    if lauf is None or lauf.status != bewertung.STATUS_FERTIG:
        return None
    erg = bewertung.lade_ergebnis(lauf)
    daten: dict = {}
    for karte in erg.karten:
        if karte.zeile:
            daten[karte.rolle] = karte.zeile
    if not daten:
        return None
    z = erg.zusammenfassung or {}
    daten["_gesamtscore"] = z.get("gesamtscore")
    daten["_anzahl_bewertet"] = z.get("anzahl_bewertet")
    return daten
...
def dashboard_rows(user: str, sort: str | None = None) -> list[dict]:
    ...
    for p in proposals.list_proposals(user):
        comp = completeness(p)
        roles = role_scores(bewertung_daten(p.slug))
...
def _role_detail(row: dict, data: dict | None, all_scores: list[int]) -> dict:
    ...
    if not isinstance(entry, dict):
        entry = {}
    criteria = ROLE_CRITERIA[row["eval_key"]]
...
def proposal_vm(proposal: proposals.Proposal | None, user: str, mode: str = "view") -> dict:
    ...
    data = bewertung_daten(proposal.slug)
    ...
    return {
        ...
        "total_state": total_state,
        "rated_count": rated,
        # Echte Quellen aus den Kapitel-17-Objekten der bewerteten Rollen
        "source_count": len({
            q
            for r_key in ("betriebsrat", "cfo", "it", "ceo")
            for q in ((data.get(r_key) or {}).get("quellen") or [])
        }) if data else 0,
        "roles": roles,
...
def knowledge_vm(user: str, sort: str = "title", dir: str = "asc", dept: str = "") -> dict:
    ...
    for page in shown:
        docs.append({
            ...
            "edits": len(stats._git_history(page, stats._history_for_root(wiki.pages_dir()))),
