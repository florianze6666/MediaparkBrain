/* Wissensgraph: Force-Directed-Graph auf Basis von /api/graph.
 *
 * Eine Logik, zwei Orte: `/graph` zeigt ihn im Vollbild, die Wissensuebersicht
 * (`/dashboard`) eingebettet. Beide rufen dieselbe Funktion:
 *
 *     MpbGraph.mount(container, { apiUrl, compact, controls, onStats })
 *
 *   container - Element, in das Canvas, Meldung und Tooltip gehaengt werden
 *   apiUrl    - Standard "/api/graph"
 *   compact   - kleinere Knoten und Labels fuer die eingebettete Ansicht
 *   controls  - Element, in dem die Bedienelemente gesucht werden
 *               ([data-graph-search], [data-graph-toggle="domain|role|similar"],
 *                [data-graph-modus])
 *   onStats   - Rueckruf mit data.stats nach jedem Laden
 *
 * Die Daten sind bereits serverseitig gefiltert (app/graph.py) - dieses Skript
 * zeigt nur an, was es bekommt, und hat keinen eigenen Zugriffsweg. Das gilt
 * auch fuer den Sichtbarkeitsmodus: der Schalter haengt nur einen Parameter an
 * die URL, entschieden wird auf dem Server (access.can_see_anonymized).
 *
...
(function () {
  ...
  function mount(wrap, options) {
    var opts = options || {};
    var apiUrl = opts.apiUrl || "/api/graph";
    var compact = !!opts.compact;
    var controls = opts.controls || document;

    if (!wrap) return null;

    // Canvas, Meldung und Tooltip anlegen, falls sie noch nicht dastehen.
    var svgEl = wrap.querySelector("svg.graph-canvas");
    if (!svgEl) {
      svgEl = document.createElementNS("http://www.w3.org/2000/svg", "svg");
      svgEl.setAttribute("class", "graph-canvas");
      svgEl.setAttribute("role", "img");
      svgEl.setAttribute("aria-label", "Graph des sichtbaren Wissens");
      wrap.appendChild(svgEl);
    }
    var messageEl = wrap.querySelector(".graph-message");
    if (!messageEl) {
    ...
    var tooltipEl = wrap.querySelector(".graph-tooltip");
    if (!tooltipEl) {
    ...
    if (!window.d3) {
      showMessage(
        ...
        "Ohne Internetzugang zum CDN bleibt die Fläche leer - die Daten liegen weiterhin unter " +
        apiUrl + "."
      );
      return null;
    }
    ...
    function nodeRadius(n) {
      var base = { page: 6, proposal: 6, domain: 11, role: 9, hidden: 5 }[n.type] || 6;
      if (compact) base = base * 0.8;
      return base + Math.min(compact ? 6 : 9, Math.sqrt(degree[n.id] || 0) * (compact ? 1.6 : 2.2));
    }

    function nodeFill(n) {
      if (n.type === "domain" || n.type === "role") return "none";
      if (n.type === "hidden") return COLOR_HIDDEN;
      if (n.type === "proposal") return COLOR_PROPOSAL;
      return domainColor[n.domaene] || PALETTE[3];
    }

    function nodeStroke(n) {
      if (n.type === "domain") return n.verborgen ? COLOR_HIDDEN : COLOR_DOMAIN;
      if (n.type === "role") return COLOR_ROLE;
      if (n.type === "hidden") return "#8b848b";
    ...
    function nodeShape(n) {
      ...
      if (n.type === "hidden") {
        return d3.symbol().type(d3.symbolSquare).size(r * r * 2.6)();
      }
    ...
    function labelVisible(n) {
      ...
      if (n.type === "hidden") return false;   // zwanzigmal "Verborgen" waere nur Rauschen
      if (n.type === "domain" || n.type === "role") return true;
      if (state.hovered) return isNear(n);
      if (state.query) return matchesQuery(n);
      return state.zoomScale >= (compact ? 1.1 : 0.8);
    }
    ...
    function tooltip(n, event) {
      ...
      var kind = {
        page: "Seite", proposal: "Projektvorschlag", domain: "Domäne",
        role: "Rolle", hidden: "Verborgenes Dokument"
      }[n.type] || n.type;
      var extra = "";
      if (n.type === "page" || n.type === "proposal" || n.type === "hidden") {
        extra = "<br>" + escapeHtml(n.domaene || "") + " · " + escapeHtml(n.vertraulichkeit || "");
      }
      if (n.type === "hidden") {
        extra += "<br><em>Kein Leserecht - nur Abteilung und Einstufung.</em>";
      }
    ...
    function fitToView() {
      if (userMoved || !state.nodes.length || !zoom) return;
      var xs = [], ys = [];
    ...
    function render(data) {
      ...
      simulation = d3.forceSimulation(state.nodes)
        .force("link", d3.forceLink(state.links)
          ...
          .distance(function (l) {
            if (l.kind === "link") return compact ? 45 : 65;
            if (l.kind === "domain") return compact ? 70 : 95;
            if (l.kind === "herkunft") return compact ? 95 : 130;
        ...
        .force("charge", d3.forceManyBody().strength(compact ? -260 : -420).distanceMax(900))
        .force("center", d3.forceCenter(dim.w / 2, dim.h / 2))
        .force("collide", d3.forceCollide().radius(function (n) {
          return nodeRadius(n) + (compact ? 11 : 18);
        }))
        .on("tick", tick);
      ...
      // Zoom / Pan. `svg.call(zoom)` ersetzt beim erneuten Rendern die Handler
      // im selben Namensraum, stapelt sie also nicht.
      zoom = d3.zoom().scaleExtent([0.2, 5]).on("zoom", function (event) {
      ...
      nodeSel
        ...
        .on("click", function (event, n) {
          if (dragged) return;           // Ziehen ist kein Klick
    ...
    function load() {
      var url = apiUrl;
      if (state.modus && state.modus !== "eigen") {
        url += (url.indexOf("?") === -1 ? "?" : "&") + "modus=" + encodeURIComponent(state.modus);
      }
      return fetch(url, { credentials: "same-origin" })
        ...
        .then(function (data) {
          render(data);
          if (typeof opts.onStats === "function") opts.onStats(data.stats || {});
        })
        .catch(function (err) {
    ...
    function bindControls() {
      var search = controls.querySelector("[data-graph-search]");
      if (search) {
      ...
      ["domain", "role", "similar"].forEach(function (name) {
        var el = controls.querySelector('[data-graph-toggle="' + name + '"]');
        if (!el) return;
        var key = { domain: "showDomain", role: "showRole", similar: "showSimilar" }[name];
      ...
      var modusEls = controls.querySelectorAll("[data-graph-modus]");
      Array.prototype.forEach.call(modusEls, function (el) {
        if (el.checked) state.modus = el.value;
        el.addEventListener("change", function () {
          if (!el.checked) return;
          state.modus = el.value;
          load();
        });
      });
    }
  ...
  window.MpbGraph = { mount: mount };

  // Beide Seiten markieren ihren Container per data-graph-mount und bekommen
  // den Graphen ohne eigenes Skript.
  function autoMount() {
    var targets = document.querySelectorAll("[data-graph-mount]");
    Array.prototype.forEach.call(targets, function (el) {
      if (el.dataset.graphMounted) return;
      el.dataset.graphMounted = "1";
      var controlsSel = el.getAttribute("data-graph-controls");
      var statsSel = el.getAttribute("data-graph-hidden-out");
      mount(el, {
        apiUrl: el.getAttribute("data-graph-api") || "/api/graph",
        compact: el.hasAttribute("data-graph-compact"),
        controls: controlsSel ? document.querySelector(controlsSel) : document,
        onStats: function (stats) {
          var out = statsSel ? document.querySelector(statsSel) : null;
          if (!out) return;
          var n = (stats && stats.hidden) || 0;
          out.textContent = n > 0
            ? "· " + n + (n === 1 ? " verborgenes Dokument" : " verborgene Dokumente") +
              " sichtbar (nur Anzahl und Abteilung)"
            : "";
        }
      });
    });
