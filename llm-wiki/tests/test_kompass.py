...
@pytest.mark.security
def test_suche_meldet_fehlende_bruecke_statt_wortsuche(client):
    """Gesucht wird seit dem 06.09.2026 semantisch im Korpus ueber qmd, nicht
    mehr per Wortabgleich in den Wiki-Seiten.

    In der Testumgebung ist die Einbettungsbruecke abgeschaltet
    (MPB_SUCHE_BRUECKE=0). Erwartet wird deshalb eine ehrliche Meldung und
    ausdruecklich KEIN stiller Rueckfall auf eine Wortsuche: die Finance-Seite
    darf auch dem CFO nicht als Treffer erscheinen, weil gar nicht gesucht wurde.
    """
    for uid in ("cfo", "mitarbeiter"):
        r = client.get("/search?q=Budget", cookies=as_user(uid))
        assert r.status_code == 200, uid
        assert "nicht verfügbar" in r.text, uid
        assert "Budget Finance" not in r.text, uid
...
@pytest.mark.skip(reason=(
    "Der Bewertungs-Cache (app/evaluation_cache.py) und die Sammelbewertung "
    "(app/evaluation.py) sind am 06.09.2026 entfallen. Bewertet wird ein "
    "benannter Antrag ueber den Orchestrator aus Phase 4; das deckt "
    "tests/test_bewertung.py mit einem Stub-Orchestrator ab."
))
def test_evaluate_schreibt_cache():
    pass
...
def test_decide_mit_vier_scores_setzt_status(client, tmp_path, monkeypatch):
    """Die vier Scores kommen aus dem letzten Orchestrator-Lauf (Phase 4),
    nicht mehr aus dem abgeloesten Bewertungs-Cache."""
    import sys
    ...
    assert _submit(client, "projektmanager", "Testprojekt").status_code == 303
    d = tmp_path / "laeufe" / "testprojekt-0001"
    laufdaten.schreibe(d, "vier")
    # wiki.json schreibt sonst die Wiki-Anwendung beim Start eines Laufs;
    # ohne sie ordnet bewertung.laeufe_fuer den Lauf keinem Antrag zu.
    import json
    (d / "wiki.json").write_text(json.dumps(
        {"slug": "testprojekt", "antraege": ["testprojekt"], "nutzer": "projektmanager"}
    ), encoding="utf-8")
    r = client.post("/proposals/testprojekt/decide",
                    cookies=as_user("projektmanager"), data={"decision": "approve"})
    assert r.status_code == 303
