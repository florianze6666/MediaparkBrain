...
def test_upload_form_get(client):
    res = client.get("/upload", cookies={"mpb_user": "projektmanager"})
...
def test_upload_docx_and_search(client, pages_env):
    ...
    # Seite laden
    page_res = client.get(redirect_url, cookies={"mpb_user": "projektmanager"})
...
def test_upload_c_level_confidentiality_isolation(client, pages_env):
    ...
    res = client.post(
        ...
        data={"vertraulichkeit": "C-Level", "domaene": "gf"},
        cookies={"mpb_user": "ceo"},
    )
    assert res.status_code == 303
    redirect_url = res.headers["location"]

    # CEO darf es lesen
    res_ceo = client.get(redirect_url, cookies={"mpb_user": "ceo"})
    assert res_ceo.status_code == 200

    # CFO darf es ebenfalls lesen (Gruppe finance/leitung)
    res_cfo = client.get(redirect_url, cookies={"mpb_user": "cfo"})
    assert res_cfo.status_code == 200

    # Normaler Mitarbeiter erhaelt 404 (US-8)
    res_mitarbeiter = client.get(redirect_url, cookies={"mpb_user": "mitarbeiter"})
